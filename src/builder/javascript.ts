...
export const javascriptBuilder: Builder = async function (bumpType: SemverBumpType, commits: RawGitCommit[], config: ResolvedGenerateNextVersionConfig) {
  const version = inc(bumpType, config)

  if (config.to !== config.base && commits.length > 0)
